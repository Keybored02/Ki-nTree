...
def compare(new_part_parameters: dict, db_part_parameters: dict, include_filters: list) -> bool:
    ...
    try:
        ...
        cprint(
            "[INFO]\tWarning: Failed to compare part with database",
...
def clean_parameter_value(category: str, name: str, value: str) -> str:
    ...
    # Sizes
    if "size" in name or "height" in name or "pitch" in name or "outline" in name:
        ...
        if len_metric > 0 and len_metric <= 1:
            ...
            # Three dimensions
            value = (
                metric[0].replace("mm", "") + "x" + metric[1].replace("mm", "") + "x" + metric[2]
            )
    ...
    # ESR, DCR, RDS
    if "esr" in name or "dcr" in name or "rds" in name:
