...
def _is_expected_filetype(filetype: str, content_type: str, data: bytes, source_url: str) -> bool:
    ...
    if (
        b"<html" in sample_lower
        or b"<!doctype html" in sample_lower
        or b"just a moment" in sample_lower
    ):
        return False
...
def _validate_downloaded_file(
    file_path: str, filetype: str, source_url: str = "", content_type: str = "", silent=False
) -> bool:
...
def _download_with_playwright(
    url: str, fileoutput: str, filetype="PDF", timeout=20, silent=False
) -> bool:
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                ...
                if _is_expected_filetype(
                    filetype, direct_response.headers.get("content-type", ""), direct_data, url
                ):
            ...
            for candidate in candidates:
                ...
                try:
                    ...
                    data = response.body()
                    if _is_expected_filetype(
                        filetype, response.headers.get("content-type", ""), data, candidate
                    ):
                        with open(fileoutput, "wb") as f:
                            f.write(data)
                        browser.close()
...
def download(
    ...
    try:
        ...
        if filetype == "Image" or filetype == "PDF":
            ...
            if requests_lib:
                ...
                if not _is_expected_filetype(filetype, content_type, response.content, url):
                    cprint(
                        f"[INFO]\tWarning: {filetype} download returned the wrong file type",
                        silent=silent,
                    )
                    return None
                ...
                response = get_image_with_retries(
                    url, headers=headers, wait=max(5, timeout), silent=silent
                )
                if not response:
                    return None
                content_type = response.headers.get("Content-Type", "")
        ...
        cprint(
            f"[INFO]\tWarning: {os.path.dirname(fileoutput)} folder does not exist", silent=silent
        )
    return None
...
def download_with_retry(url: str, full_path: str, silent=False, **kwargs) -> str:
    ...
    if filetype in ["Image", "PDF"]:
        ...
        if _download_with_playwright(
            url=url, fileoutput=full_path, filetype=filetype, timeout=timeout, silent=silent
        ):
            return True
    ...
    if not file:
        # Try with headers
        file = download(
            url, fileoutput=full_path, enable_headers=True, silent=silent, **download_kwargs
        )
