...
def _get_api():
    ...
    cprint(f'[PICKUP]\tAPI object: {type(api).__name__ if api else "None"}, token set: {bool(api)}', silent=False)
    return api
...
def _resolve_locations(api, part_pk: int) -> List[str]:
    ...
    try:
        ...
        cprint(f'[PICKUP]\t  stock items for part {part_pk}: {len(stock_items)}', silent=False)
    ...
    if not locations:
        try:
            ...
            cprint(f'[PICKUP]\t  default_location fallback failed for part {part_pk}: {exc}', silent=False)
...
def _collect_bom(
    ...
    cprint(f'[PICKUP]\t  BOM depth={depth} part={part_pk}: {len(bom_items)} item(s)', silent=False)
    ...
    for item in bom_items:
        ...
        try:
            ...
            sub_name = str(
                getattr(sub_part, 'name', '')
                or getattr(sub_part, 'IPN', '')
                or str(sub_pk)
            )
        except Exception as exc:
        ...
        if is_assembly and depth < MAX_DEPTH and sub_pk not in visited_parts:
            cprint(f'[PICKUP]\t  sub-assembly {sub_pk} ({sub_name}), recursing…', silent=False)
