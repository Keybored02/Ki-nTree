...
class BarcodeScannedRow:
    ...
    def __init__(self, barcode: str, parsed: Dict):
        ...
        self.manufacturer_pn = (
            parsed.get("manufacturer_pn", "")
            or parsed.get("product_code", "")
            or parsed.get("digikey_pn", "")
        )
        ...
        self.search_name = (
            self.manufacturer_pn or self.supplier_pn
            if self.supplier == "mouser"
            else self.supplier_pn or self.manufacturer_pn
        )

        # Match Assign display behavior: show resolved lookup for known suppliers.
        self.lookup_value = (
            parsed.get("barcode", "") or self.manufacturer_pn or self.supplier_pn or barcode
        )
        self.display_code = (
            self.lookup_value if self.supplier != "unknown" and self.lookup_value else barcode
        )
...
class _BarcodeApiHelpers:
    ...
    def resolve_location_string(part: Dict) -> str:
        ...
        try:
            ...
            names = [
                str(name) for name in reversed(list(location_tree.values())) if str(name).strip()
            ]
            if names:
                return "/".join(names)
        except Exception:
            ...
            logging.exception("Exception building location path:")
    ...
    def fetch_part_barcodes(ctx, part_pk: int) -> List[str]:
        ...
        if getattr(ctx, "_barcode_endpoint_available", None) is None:
            try:
                ...
                ctx._barcode_endpoint_available = probe_response.status_code not in [
                    404,
                    405,
                ]
            except Exception:
        ...
        try:
            ...
            logging.exception("Exception extracting barcode values:")
    ...
    @staticmethod
    def collect_transfer_items_for_part(
        ctx, part_pk: int, location_pk: int
    ) -> tuple[List[Dict], str]:
        api_obj = getattr(inventree_interface.inventree_api, "inventree_api", None)
        if not api_obj:
            ...
            while url:
                ...
                for item in rows:
                    ...
                    try:
                        ...
                        logging.exception("Exception comparing current_loc and location_pk:")
                    ...
                    try:
                        ...
                        logging.exception("Exception appending transfer item:")
                        continue
    ...
    @staticmethod
    def transfer_stock_items_bulk(
        ctx, transfer_items: List[Dict], location_pk: int
    ) -> tuple[bool, str]:
        ...
        if transfer_response is not None and transfer_response.status_code in [
            200,
            201,
            202,
        ]:
    ...
    def _extract_pk(entity: Optional[Dict]) -> int:
        if not isinstance(entity, dict):
            # The following block is skipped since api_obj and part_pk are undefined in this context
            pass
        # Safely get 'key' and 'probe' from entity or default to empty string
        value = str(entity.get("key", "")).strip().lower()
    ...
    def resolve_supplier_company_pk(ctx, supplier_key: str) -> int:
            ...
            if cached_pk > 0:
                try:
                    ...
                    if cached_pk in current_companies.values():
                        cprint(
                            f"[BARCODE][PO]\tSupplier cache hit validated: {supplier_norm} -> pk={cached_pk}",
                            silent=False,
                        )
                        return cached_pk
                    cprint(
                        f"[BARCODE][PO]\tSupplier cache stale: {supplier_norm} -> pk={cached_pk}",
                        silent=False,
                    )
                except Exception as exc:
                    cprint(
                        f"[BARCODE][PO]\tSupplier cache validation failed for {supplier_norm}: {str(exc)[:80]}",
                        silent=False,
                    )
        ...
        def _pick_company(probe: str) -> int:
            ...
            if exact_matches:
                ...
                cprint(
                    f"[BARCODE][PO]\tSupplier exact match: {probe} -> {company_name} pk={company_pk}",
                    silent=False,
                )
                return company_pk
            ...
            if contains_matches:
                ...
                cprint(
                    f"[BARCODE][PO]\tSupplier contains match: {probe} -> {company_name} pk={company_pk}",
                    silent=False,
                )
                return company_pk
    ...
    def _pick_po_endpoint(ctx, for_lines: bool = False) -> str:
        ...
        candidates = (
            ["/api/order/po-line/", "/api/order/purchase-line/"]
            if for_lines
            else ["/api/order/po/", "/api/order/purchase/"]
        )
        ...
        for path in candidates:
            ...
            if response is not None:
                ...
                cprint(
                    f"[BARCODE][PO]\tResolved {'line' if for_lines else 'header'} endpoint: {endpoint}",
                    silent=False,
                )
                return endpoint
    ...
    def create_purchase_order(
        ctx, supplier_pk: int, reference: str, location_pk: int = 0
        ...
        for payload_index, payload in enumerate(payloads, start=1):
                ...
                try:
                    ...
                    if response.status_code in [200, 201, 202]:
                        ...
                        try:
                            ...
                            logging.exception("Exception parsing created barcode response:")
                        return {"pk": 0}, ""
            ...
            cprint(
                f"[BARCODE][PO]\tCreate PO attempt {payload_index} failed: {detail}",
                silent=False,
            )
        ...
        for order_key in ["order", "purchase_order", "po"]:
            ...
            for item in _BarcodeApiHelpers._extract_rows(response.json()):
                ...
                if isinstance(part_detail, dict):
                    raw_internal = (
                        part_detail.get("part")
                        or part_detail.get("part_pk")
                        or part_detail.get("part_id")
                    )
    ...
    def create_po_line(ctx, po_pk: int, part_pk: int, quantity: int) -> tuple[Optional[Dict], str]:
        ...
        cprint(
            f"[BARCODE][PO]\tCreate line: endpoint={endpoint} po={po_pk} part={part_pk} qty={qty}",
            silent=False,
        )
        ...
        for payload_index, payload in enumerate(payloads, start=1):
            response = None
            cprint(
                f"[BARCODE][PO]\tCreate line attempt {payload_index}/{len(payloads)} payload={payload}",
                silent=False,
            )
            for request_try in range(1, 4):
                try:
            ...
            detail = _BarcodeApiHelpers._response_excerpt(response)
            cprint(
                f"[BARCODE][PO]\tCreate line attempt {payload_index} failed: {detail}",
                silent=False,
            )
    ...
    def update_po_line_quantity(ctx, line_pk: int, quantity: int) -> tuple[bool, str]:
        ...
        cprint(
            f"[BARCODE][PO]\tUpdate line: endpoint={line_endpoint} line={line_pk} qty={qty}",
            silent=False,
        )
    ...
    def receive_po_items(
        ctx, po_pk: int, items: List[Dict], location_pk: int = 0
    ) -> tuple[bool, str]:
        if po_pk <= 0:
        ...
        payloads = [
            {"items": normalized_items, "location": int(location_pk)}
            if location_pk > 0
            else {"items": normalized_items},
        ]

        for endpoint in endpoint_candidates:
            for payload in payloads:
                cprint(
                    f"[BARCODE][PO]\tReceive PO attempt endpoint={endpoint} payload={payload}",
                    silent=False,
                )
                response = ctx._request_with_retries(
                    method="POST",
                ...
                if response.status_code in [200, 201, 202]:
                    return True, ""
                cprint(
                    f"[BARCODE][PO]\tReceive PO failed: {_BarcodeApiHelpers._response_excerpt(response)}",
                    silent=False,
                )
...
class BarcodeImportView(MainView):
    ...
    def _on_barcode_input_changed(self, _):
        ...
        if success:
            ...
            if failed:
                self.show_dialog(
                    DialogType.ERROR,
                    f"Unrecognized barcode format for {failed} item(s)",
                )
        else:
            self._show_status("Unknown barcode format", color="red")
            self.show_dialog(DialogType.ERROR, "Unrecognized barcode format")

    def _append_parsed_barcode(
        self, barcode: str, update_table: bool = True, update_status: bool = True
    ) -> bool:
    ...
    def _parse_batch_barcodes(self, _):
        ...
        self.fields["barcode_input"].update()
        self._show_status(
            f"Parsed {success} items ({failed} failed)",
    ...
    def _update_results_table(self):
        ...
        for idx, row in enumerate(self.scanned_rows):
            ...
            input_code = self._truncate_text(
                str(getattr(row, "display_code", "") or row.raw_barcode or "-"),
                max_len=70,
            )
            ...
            if part_display and part_pk > 0:
                ...
                if str(getattr(row, "status", "")).lower().startswith("checking"):
                    ...
                    part_text = str(
                        row.search_name or row.manufacturer_pn or row.supplier_pn or "-"
                    )
            ...
            if row.supplier != "unknown":
                qty_cell_content = ft.TextField(
                    value=str(
                        row._edited_quantity
                        if row._edited_quantity is not None
                        else (row.quantity or 1)
                    ),
                    width=80,
                    dense=True,
            ...
            if row.supplier != "unknown":
                order_display = (
                    row._edited_order_number
                    if row._edited_order_number is not None
                    else (row.order_number or "")
                )
            ...
            if status_lower.startswith("existing"):
                ...
                status_lower.startswith("skipped")
                or status_lower.startswith("error")
                or status_lower.startswith("failed")
            ):
                status_color = "red"
            else:
                status_color = "orange"
            ...
            if row.category:
                ...
                category_cell_content = ft.Container(
                    ...
                    content=ft.AutoComplete(
                        suggestions=[
                            ft.AutoCompleteSuggestion(key=c, value=c) for c in all_cat_options
                        ],
    ...
    def _find_supplier_part_for_row(
        ...
        supplier_norm = str(supplier_key or "").strip().lower()
        supplier_aliases = _BarcodeApiHelpers.PO_SUPPLIER_NAME_MAP.get(
            supplier_norm, [supplier_key]
        )
        ...
        if supplier_pk <= 0:
            cprint(
                f"[BARCODE][PO]\tUnable to resolve supplier company for {supplier_norm}",
                silent=False,
            )
            return None
                ...
                for candidate in rows:
                    ...
                    if not supplier_name:
                        ...
                        if isinstance(supplier_ref, dict):
                            supplier_name = (
                                str(
                                    supplier_ref.get("name")
                                    or supplier_ref.get("display_name")
                                    or ""
                                )
                                .strip()
                                .lower()
                    ...
                    if supplier_aliases and supplier_name:
                        if not any(
                            alias.lower() == supplier_name or alias.lower() in supplier_name
    ...
    def _resolve_supplier_part_pk(
        self, supplier_pk: int, part_pk: int, lookup_values: Optional[List[str]] = None
    ) -> int:
    ...
    def _create_supplier_part_link(
        self, supplier_pk: int, part_pk: int, lookup_values: Optional[List[str]] = None
    ) -> int:
        if int(supplier_pk or 0) <= 0 or int(part_pk or 0) <= 0:
            return 0
            ...
            for payload in payloads:
                ...
                if response.status_code in [200, 201, 202]:
                    try:
                        created = response.json()
                        created_pk = int(
                            (created or {}).get("pk") or (created or {}).get("id") or 0
                        )
                    except Exception:
                        created_pk = 0
                    if created_pk > 0:
        ...
        return self._resolve_supplier_part_pk(
            supplier_pk=supplier_pk, part_pk=part_pk, lookup_values=lookup_values
        )
        ...
        for probe in probe_values:
            ...
            if part_match:
                ...
                if part_pk > 0 and self._validate_part_pk(part_pk):
                    ...
                    if isinstance(part_detail, dict):
                        part_detail["supplier_part_pk"] = int(
                            part_match.get("supplier_part_pk") or 0
                        )
                        return part_detail
            ...
            for probe in probe_values:
                ...
                for candidate in rows:
                    ...
                    return {
                        "pk": part_pk,
                        "part_pk": part_pk,
                        "supplier_part_pk": supplier_part_pk,
                    }
        except Exception:
            return None
    ...
    def _validate_import_row_async(self, row: BarcodeScannedRow):
        ...
        try:
            ...
            if isinstance(category_detail, dict):
                part_category = str(
                    category_detail.get("pathstring") or category_detail.get("name") or ""
                ).strip()
            ...
            if not part_category:
                ...
                if isinstance(raw_category, dict):
                    part_category = str(
                        raw_category.get("pathstring") or raw_category.get("name") or ""
                    ).strip()
    ...
    def _refresh_import_button_state(self):
        ...
        if not self.scanned_rows:
            ...
            global_category = (
                self.fields["category_select"].value if self.fields.get("category_select") else None
            )
            all_have_category = all(
                bool(row.category) or bool(global_category) for row in self.scanned_rows
            )
    ...
    def _get_stock_location_pk(self, location: str) -> int:
        location_pk = inventree_interface.resolve_stock_location_pk(
            location, self.stock_location_id_map
        )
        if location_pk > 0:
            cprint(
                f"[BARCODE]\tResolved stock location: {location} -> pk={location_pk}",
                silent=False,
            )
        else:
    ...
    def _update_existing_part_metadata(
        ...
        if new_name and db_name.lower() != new_name.lower():
            try:
                ...
                cprint(
                    f'[BARCODE]\tUpdated part name (MPN): "{db_name}" → "{new_name}" (pk={part_pk})',
                    silent=False,
                )
            except Exception as exc:
                cprint(
                    f"[WARN]\tFailed to update part name for pk={part_pk}: {exc}",
                    silent=False,
                )
                ...
                if matched_mfr:
                    ...
                    if db_mpn.lower() != new_mpn.lower():
                        ...
                        if mfr_pk:
                            ...
                            if patch_resp is not None and patch_resp.status_code in [
                                200,
                                202,
                            ]:
                                cprint(
                                    f'[BARCODE]\tUpdated MPN: "{db_mpn}" → "{new_mpn}" (mfr_part_pk={mfr_pk})',
                                    silent=False,
                                )
                            else:
                                cprint(
                                    f"[WARN]\tFailed to update MPN for mfr_part_pk={mfr_pk}",
                                    silent=False,
                                )
                else:
                    ...
                    try:
                        ...
                        cprint(
                            f"[BARCODE]\tCreated manufacturer part MPN={new_mpn} for pk={part_pk}",
                            silent=False,
                        )
                    except Exception as exc:
                        cprint(
                            f"[WARN]\tCould not create manufacturer part for pk={part_pk}: {exc}",
                            silent=False,
                        )
            except Exception as exc:
                cprint(
                    f"[WARN]\tManufacturer part reconciliation error for pk={part_pk}: {exc}",
                    silent=False,
                )
            ...
            try:
                ...
                if this_supplier_parts:
                    ...
                    exact_match = next(
                        (
                            item
                            for item in this_supplier_parts
                    ...
                    if exact_match:
                        ...
                        if sup_pk:
                            ...
                            if patch_resp is not None and patch_resp.status_code in [
                                ...
                                cprint(
                                    f'[BARCODE]\tUpdated SKU: "{db_sku}" → "{new_sku}" (sup_part_pk={sup_pk})',
                                    silent=False,
                                )
                            else:
                                cprint(
                                    f"[WARN]\tFailed to update SKU for sup_part_pk={sup_pk}",
                                    silent=False,
                                )
                    else:
                ...
                cprint(
                    f"[WARN]\tSupplier part reconciliation error for pk={part_pk}: {exc}",
                    silent=False,
                )
        ...
        if use_manufacturer_barcode:
            ...
            if barcode_target:
                ...
                current_normalized = {
                    str(b or "").strip().lower() for b in current_barcodes if str(b or "").strip()
                }
                if barcode_target.lower() not in current_normalized:
                    for attempt in range(1, 4):
                        try:
                            ok = self._link_part_barcode(
                                part_pk=part_pk, barcode_value=barcode_target
                            )
                            if ok:
                                break
                        except Exception as exc:
                            cprint(
                                f"[WARN]\tBarcode link retry {attempt} for pk={part_pk}: {exc}",
                                silent=False,
                            )
                        if attempt < 3:
                            time.sleep(0.5)
            ...
            if new_sku and part_pk:
                try:
                    ...
                    if not ok:
                        cprint(
                            f"[WARN]\tSupplier part barcode assignment failed for pk={part_pk} sku={new_sku}",
                            silent=False,
                        )
                except Exception as exc:
                    cprint(
                        f"[WARN]\tSupplier part barcode error for pk={part_pk}: {exc}",
                        silent=False,
                    )

    def _collect_transfer_items_for_part(
        self, part_pk: int, location_pk: int
    ) -> tuple[List[Dict], str]:
        return _BarcodeApiHelpers.collect_transfer_items_for_part(self, part_pk, location_pk)

    def _transfer_stock_items_bulk(
        self, transfer_items: List[Dict], location_pk: int
    ) -> tuple[bool, str]:
    ...
    def _create_purchase_order(
        self, supplier_pk: int, reference: str, location_pk: int = 0
    ) -> tuple[Optional[Dict], str]:
    ...
    def _create_po_line(
        self, po_pk: int, part_pk: int, quantity: int
    ) -> tuple[Optional[Dict], str]:
    ...
    def _receive_po_line(
        self, line_pk: int, quantity: int, location_pk: int = 0
    ) -> tuple[bool, str]:
        return _BarcodeApiHelpers.receive_po_items(
            self, line_pk, [{"line_item": line_pk, "quantity": quantity}], location_pk
        )

    def _receive_po_items(
        self, po_pk: int, items: List[Dict], location_pk: int = 0
    ) -> tuple[bool, str]:
    ...
    def _on_submit(self, _):
        ...
        missing_category = [
            row for row in self.scanned_rows if not row.category and not global_category
        ]
        if missing_category:
            self.show_dialog(
                DialogType.ERROR,
    ...
    def _execute_import(self):
        ...
        if not self._connect_server_with_retries(attempts=3, delay_seconds=1.5):
            cprint(
                "[BARCODE]\tImport aborted: failed to connect to InvenTree",
                silent=False,
            )
            self.fields[
                "import_progress_message"
            ].value = "Import failed: server offline after retries"
            self.fields["import_progress_message"].color = "red"
            self.fields["import_progress_message"].update()
            self.show_dialog(
                DialogType.ERROR,
                "Failed to connect to InvenTree server after 3 retries",
            )
            return
        ...
        assign_all_stock_items_location = (
            bool(self.fields.get("assign_all_stock_items_location_check").value)
            and not po_flow_enabled
        )
        ...
        if assign_location_existing:
            ...
            if selected_location_pk <= 0:
                if po_flow_enabled:
                    cprint(
                        f"[BARCODE]\tSelected location ignored for PO flow: {selected_location_value}",
                        silent=False,
                    )
                    selected_location_pk = 0
                else:
                    self.fields[
                        "import_progress_message"
                    ].value = "Import failed: selected location could not be resolved"
                    self.fields["import_progress_message"].color = "red"
                    self.fields["import_progress_message"].update()
                    self.show_dialog(
                        DialogType.ERROR,
                        f"Selected stock location not found: {selected_location_value}",
                    )
                    return
            ...
            try:
                ...
                if existing_part:
                    ...
                    if assign_location_existing and not po_flow_enabled:
                        set_ok = self._set_part_default_location(
                            part_pk=part_pk, location_pk=selected_location_pk
                        )
                        if not set_ok:
                    ...
                    if po_flow_enabled:
                        if str(row.supplier or "").strip().lower() == "unknown":
                            result["ok"] = False
                            result["failure"] = (
                                f"{row.search_name}: PO flow requires known supplier"
                            )
                            return result
                        if not result["po_reference"]:
                            result["ok"] = False
                            result["failure"] = (
                                f"{row.search_name}: missing supplier order reference for PO flow"
                            )
                            return result
                    return result
                ...
                if str(row.supplier or "").strip().lower() != "unknown":
                    ...
                    result["failure"] = (
                        f"{row.search_name}: skipped new part creation (unknown supplier)"
                    )
                    return result

                category_tree = (
                    inventree_interface.split_category_tree(row.category) if row.category else []
                )
                ...
                if not part_pk:
                    ...
                    if existing_after_create:
                        part_pk = int(
                            existing_after_create.get("pk") or existing_after_create.get("id") or 0
                        )
                ...
                if barcode_value:
                    try:
                        inventree_interface.inventree_api.link_barcode(
                            barcode_value, part_pk=part_pk
                        )
                    except Exception as exc:
                        cprint(
                            f"[WARN]\tBarcode linking failed for {row.search_name}: {str(exc)}",
                            silent=False,
                        )
                ...
                if use_manufacturer_barcode and row.supplier_pn and part_pk:
                    try:
                        ...
                        if not ok:
                            cprint(
                                f"[WARN]\tSupplier part barcode assignment failed for {row.search_name} (sku={row.supplier_pn})",
                                silent=False,
                            )
                    except Exception as exc:
                        cprint(
                            f"[WARN]\tSupplier part barcode error for {row.search_name}: {str(exc)[:60]}",
                            silent=False,
                        )
                ...
                cprint(
                    f"[WARN]\tImport exception for {item_label}: {str(exc)[:120]}",
                    silent=False,
                )
                return result
        ...
        if po_flow_enabled:
            po_candidates = [
                item for item in row_results if item.get("ok") and int(item.get("part_pk") or 0) > 0
            ]
            cprint(
                f"[BARCODE][PO]\tPO flow enabled: candidates={len(po_candidates)}",
                silent=False,
            )
            ...
            for item in po_candidates:
                ...
                if not supplier_key or not po_ref:
                    item["ok"] = False
                    item["failure"] = (
                        f"{item['row'].search_name}: missing supplier/order reference for PO flow"
                    )
                    cprint(
                        f"[BARCODE][PO]\tMissing reference row={item['row'].search_name} supplier={supplier_key or '-'}",
                        silent=False,
                    )
                    continue
            ...
            cprint(
                f"[BARCODE][PO]\tGrouped into {len(po_groups)} supplier/order buckets",
                silent=False,
            )
            ...
            for (supplier_key, _), group in po_groups.items():
                ...
                if open_pos:
                    ...
                    cprint(
                        f"[BARCODE][PO]\tOpen editable POs for supplier_pk={supplier_pk}: none",
                        silent=False,
                    )
                ...
                for item in group["items"]:
                    ...
                    if supplier_part_pk <= 0:
                        ...
                        if part_pk > 0:
                            group_data = unresolved_groups.setdefault(
                                part_pk, {"qty": 0, "items": []}
                            )
                            ...
                            cprint(
                                f"[BARCODE][PO]\tMissing supplier_part_pk for row={item['row'].search_name} part_pk={part_pk}",
                                silent=False,
                            )
                            item["ok"] = False
                            item["failure"] = (
                                f"{item['row'].search_name}: missing supplier part for PO flow"
                            )
                        continue

                    group_parts[supplier_part_pk] = group_parts.get(supplier_part_pk, 0) + int(
                        item.get("po_quantity") or 1
                    )
                ...
                if not po_data:
                    ...
                    if not po_data:
                        ...
                        cprint(
                            f"[BARCODE][PO]\tCreate PO failed supplier_pk={supplier_pk} ref={po_ref}: {detail}",
                            silent=False,
                        )
                        for item in group["items"]:
                            item["ok"] = False
                            item["failure"] = (
                                f"{item['row'].search_name}: PO flow failed ({detail})"
                            )
                        continue
                    po_created = True
                    cprint(
                        f"[BARCODE][PO]\tCreated new PO supplier_pk={supplier_pk} ref={po_ref}",
                        silent=False,
                    )
                else:
                    cprint(
                ...
                if po_pk <= 0:
                    cprint(
                        f"[BARCODE][PO]\tInvalid PO id supplier_pk={supplier_pk} ref={po_ref}",
                        silent=False,
                    )
                    for item in group["items"]:
                        item["ok"] = False
                        item["failure"] = (
                            f"{item['row'].search_name}: PO flow failed (invalid PO id)"
                        )
                    continue

                receive_location_pk = self._get_po_location_pk(po_data) or int(
                    selected_location_pk or 0
                )
                ...
                if not group_parts and not unresolved_groups:
                    cprint(
                        f"[BARCODE][PO]\tSkipping PO operations (no resolvable lines) supplier={supplier_key} ref={po_ref}",
                        silent=False,
                    )
                    continue
                ...
                if po_issued:
                    ...
                    cprint(
                        f"[BARCODE][PO]\tIssue step failed or PO already issued pk={po_pk}",
                        silent=False,
                    )
                ...
                for supplier_part_pk, qty in group_parts.items():
                    ...
                    if not po_line:
                        po_line, line_error = self._create_po_line(
                            po_pk=po_pk, part_pk=supplier_part_pk, quantity=qty
                        )
                        if not po_line:
                            part_errors[supplier_part_pk] = line_error or "failed to create PO line"
                            cprint(
                                f"[BARCODE][PO]\tCreate line failed po={po_pk} supplier_part={supplier_part_pk}: {part_errors[supplier_part_pk]}",
                                silent=False,
                            )
                            continue
                        cprint(
                            f"[BARCODE][PO]\tCreated line po={po_pk} supplier_part={supplier_part_pk} qty={qty}",
                            silent=False,
                        )
                    ...
                    if line_pk <= 0:
                        ...
                        cprint(
                            f"[BARCODE][PO]\tInvalid line id po={po_pk} supplier_part={supplier_part_pk}",
                            silent=False,
                        )
                        continue
                    ...
                    if required_order_qty > int(ordered_qty or 0):
                        ...
                        if not updated:
                            part_errors[supplier_part_pk] = (
                                update_error or "failed to update PO line quantity"
                            )
                            cprint(
                                f"[BARCODE][PO]\tUpdate line qty failed line={line_pk} required={required_order_qty}: {part_errors[supplier_part_pk]}",
                                silent=False,
                            )
                            continue
                        cprint(
                            f"[BARCODE][PO]\tUpdated line={line_pk} ordered={ordered_qty}->{required_order_qty}",
                            silent=False,
                        )
                ...
                for internal_part_pk, group_data in unresolved_groups.items():
                    ...
                    po_line = self._find_po_line_for_internal_part(
                        po_pk=po_pk, internal_part_pk=int(internal_part_pk)
                    )
                    if not po_line:
                    ...
                    if required_order_qty > int(ordered_qty or 0):
                        ...
                        if not updated:
                            ...
                            cprint(
                                f"[BARCODE][PO]\tUpdate line qty failed line={line_pk} required={required_order_qty}: {detail}",
                                silent=False,
                            )
                            for item in group_data.get("items") or []:
                                unresolved_item_errors[int(item.get("idx") or 0)] = detail
                ...
                if not part_errors and receive_items:
                    ...
                    if not received:
                        ...
                        cprint(
                            f"[BARCODE][PO]\tReceive failed po={po_pk}: {detail}",
                            silent=False,
                        )
                        for item in group["items"]:
                            item["ok"] = False
                    ...
                    cprint(
                        f"[BARCODE][PO]\tReceived PO pk={po_pk} items={len(receive_items)} location={receive_location_pk}",
                        silent=False,
                    )
                ...
                for item in group["items"]:
                    ...
                    if supplier_part_pk in part_errors:
                        item["ok"] = False
                        item["failure"] = (
                            f"{item['row'].search_name}: PO flow failed ({part_errors[supplier_part_pk]})"
                        )
                    item_idx = int(item.get("idx") or 0)
                    if item_idx in unresolved_item_errors:
                        item["ok"] = False
                        item["failure"] = (
                            f"{item['row'].search_name}: PO flow failed ({unresolved_item_errors[item_idx]})"
                        )
        ...
        if assign_all_stock_items_location and selected_location_pk > 0:
            existing_success_rows = [
                ...
                if item.get("ok")
                and item.get("existing_part")
                and int(item.get("part_pk") or 0) > 0
            ]
            ...
            if bulk_ok:
                cprint(
                    f"[BARCODE]\tStock items updated (items={len(all_transfer_items)}, parts={len(part_to_rows)})",
                    silent=False,
                )
            else:
                cprint(
                    "[BARCODE]\tCombined stock transfer failed for existing parts",
                    silent=False,
                )
                ...
                for item in existing_success_rows:
                    item["ok"] = False
                    item["failure"] = (
                        f"{item['row'].search_name}: stock location assignment failed ({detail})"
                    )
        ...
        if failed == 0:
            ...
            try:
                ...
                logging.exception("Exception updating category/location fields after import:")
        else:
        ...
        self.fields["import_progress"].color = (
            "green" if failed == 0 else ("amber" if success > 0 else "red")
        )
        self.fields[
            "import_progress_message"
        ].value = f"Import finished: {success} success, {failed} failed"
        ...
        cprint(
            f"[BARCODE]\tTotal operation time: {elapsed_total:.1f} ms ({total} rows)",
            silent=False,
        )
        if total:
            cprint(
                f"[BARCODE]\tAverage per row: {elapsed_total / total:.1f} ms",
                silent=False,
            )
        cprint(
            f"[BARCODE]\tImport finished: success={success} failed={failed}",
            silent=False,
        )

        self._show_status(
            f"Import complete: {success} success, {failed} failed",
            color="green" if failed == 0 else "orange",
        )
...
class BarcodeAssignmentView(MainView):
                ...
                ft.Container(
                    content=ft.Column(
                        controls=[
                            ...
                            ft.Divider(),
                            ft.Text(
                                "2. Review Existing Part Matches:",
                                style=ft.TextThemeStyle.BODY_LARGE,
                            ),
    ...
    def _validate_row_async(self, row_id: int):
        ...
        try:
            ...
            cprint(
                f"[ASSIGN]\tValidation row {row_id}: server check {elapsed_server:.1f} ms",
                silent=False,
            )
            ...
            if not part:
                ...
                cprint(
                    f"[ASSIGN]\tValidation row {row_id}: part lookup failed {elapsed_part_lookup:.1f} ms",
                    silent=False,
                )
            else:
                cprint(
                    f"[ASSIGN]\tValidation row {row_id}: part lookup {elapsed_part_lookup:.1f} ms",
                    silent=False,
                )
                ...
                cprint(
                    f"[ASSIGN]\tValidation row {row_id}: location resolve {elapsed_location:.1f} ms",
                    silent=False,
                )
                ...
                cprint(
                    f"[ASSIGN]\tValidation row {row_id}: barcode ops {elapsed_barcodes:.1f} ms",
                    silent=False,
                )

                has_location = bool(
                    row.location and row.location != "-" and row.location.lower() != "none"
                )
                if has_location and row.has_barcode:
        ...
        cprint(
            f"[ASSIGN]\tValidation row {row_id} total: {elapsed_total:.1f} ms",
            silent=False,
        )
    ...
    def _get_stock_location_pk(self, location: str) -> int:
        location_pk = inventree_interface.resolve_stock_location_pk(
            location, self._location_path_to_pk_cache
        )
        if location_pk > 0:
            cprint(
                f"[ASSIGN]\tResolved stock location from cache: {location} -> pk={location_pk}",
                silent=False,
            )
        else:
    ...
    def _update_all_stock_items_location(
        self, part_pk: int, location_pk: int
    ) -> tuple[int, int, str]:
        ...
        try:
            ...
            cprint(
                "[ASSIGN]\tBulk stock transfer unavailable, falling back to per-item PATCH",
                silent=False,
            )
            for item in stock_items:
                item_pk = item.get("pk") or item.get("id")
        ...
        for row in rows_snapshot:
            status_color = (
                ...
                if row.status.startswith("Valid")
                    ...
                    "red"
                    if "not found" in row.status.lower() or "error" in row.status.lower()
                    else "blue"
                )
            )
            if row.current_barcodes:
    ...
    def _apply_assignments(self, _):
        ...
        apply_stock_items_location = bool(
            self.fields["assign_all_stock_items_location_check"].value
        )
        ...
        if apply_location:
            ...
            if not location_value:
                ...
                return
            cprint(
                f"[ASSIGN]\tUsing selected stock location: {location_value}",
                silent=False,
            )
            ...
            cprint(
                f"[ASSIGN]\tInitial stock location resolve: pk={location_pk} ({elapsed_loc_resolve:.1f} ms)",
                silent=False,
            )
            ...
            try:
                if apply_location:
                    ...
                    if location_pk > 0:
                        ...
                        set_ok = inventree_interface.inventree_set_part_default_location(
                            row.part_pk, location_value
                        )
                    elapsed_loc_update = (time.perf_counter() - ts_loc_update) * 1000.0
                    cprint(
                        f"[ASSIGN]\t  Row {idx}: part_default_location update ({elapsed_loc_update:.1f} ms)",
                        silent=False,
                    )
                    ...
                    if apply_stock_items_location and location_pk <= 0:
                        ...
                        location_stage_ok = False
                        row_failures.append(
                            f"{row.part_name}: stock location assignment failed (invalid location)"
                        )

                if reassign_barcode and row.part_name:
                    if row.has_barcode and not force_reassign:
                        cprint(
                            f"[ASSIGN]\t  Row {idx}: barcode exists, skipped",
                            silent=False,
                        )
                    else:
                        ...
                        if row.part_name.strip().lower() in current_barcodes_normalized:
                            cprint(
                                f"[ASSIGN]\t  Row {idx}: barcode already matches part name, skipped",
                                silent=False,
                            )
                        else:
                            ...
                            cprint(
                                f"[ASSIGN]\t  Row {idx}: barcode link ({elapsed_barcode:.1f} ms)",
                                silent=False,
                            )
                            if not barcode_ok:
                                row_ok = False
        ...
        if apply_stock_items_location and location_pk > 0:
            ...
            for part_pk in sorted(part_to_rows.keys()):
                part_items, part_error = self._collect_transfer_items_for_part(
                    part_pk=part_pk, location_pk=location_pk
                )
                if part_error:
                    collect_errors.append(f"part_pk={part_pk}: {part_error}")
                if part_items:
            ...
            if bulk_ok:
                ...
                cprint(
                    "[ASSIGN]\tCombined stock transfer failed, falling back to per-part updates",
                    silent=False,
                )
                for part_pk in sorted(part_to_rows.keys()):
                    ...
                    if stock_failed > 0 or stock_error:
                        details = (
                            stock_error
                            if stock_error
                            else f"{stock_failed} stock item updates failed"
                        )
                            ...
                            for result in row_results:
                                if result["row"].row_id == row.row_id:
                                    ...
                                    result["failures"].append(
                                        f"{row.part_name}: stock location assignment failed ({details})"
                                    )
        ...
        self.fields[
            "progress_message"
        ].value = f"Update finished: {success} success, {failed} failed"
        ...
        elapsed_total = (time.perf_counter() - op_start_ts) * 1000.0
        cprint(
            f"[ASSIGN]\tTotal operation time: {elapsed_total:.1f} ms ({total} rows)",
            silent=False,
        )
        ...
        if successful_row_ids:
            with self._rows_lock:
                self.scanned_rows = [
                    row for row in self.scanned_rows if row.row_id not in successful_row_ids
                ]
            self._update_results_table()

        self._set_status(
            f"Update finished: {success} success, {failed} failed",
            color="green" if failed == 0 else "orange",
        )
