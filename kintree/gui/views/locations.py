...
class LocationsView(MainView):
            ...
            controls=[
                ft.Container(
                    content=ft.Column(
                        controls=[
                            # Section 1
                            ft.Text('Reassign Locations', style=ft.TextThemeStyle.HEADLINE_SMALL),
                            ft.Row([
                            ...
                            # Section 2
                            ft.Text('Move Items to Location', style=ft.TextThemeStyle.HEADLINE_SMALL),
    ...
    def _request_with_retries(self, method: str, url: str, attempts: int = 3,
                               delay_seconds: float = 1.0, **kwargs) -> Optional[requests.Response]:
        for attempt in range(1, attempts + 1):
            try:
    ...
    def _on_move(self, _):
        ...
        source_pk = inventree_interface.resolve_stock_location_pk(source, self.stock_location_id_map)
        if source_pk <= 0:
        ...
        if target == self._ROOT_LABEL:
            ...
            parent_pk = inventree_interface.resolve_stock_location_pk(target, self.stock_location_id_map)
            if parent_pk <= 0:
    ...
    def _update_results_table(self):
        ...
        for row in rows_snapshot:
            ...
            table_rows.append(ft.DataRow(cells=[
                ft.DataCell(ft.Text(self._truncate(row.display_code), size=12, no_wrap=True, overflow=ft.TextOverflow.ELLIPSIS)),
                ft.DataCell(ft.Text(self._truncate(row.supplier.upper()), size=12, no_wrap=True, overflow=ft.TextOverflow.ELLIPSIS)),
                ft.DataCell(ft.Text(self._truncate(row.lookup_value), size=12, no_wrap=True, overflow=ft.TextOverflow.ELLIPSIS)),
                ft.DataCell(ft.Text(self._truncate(row.status), color=status_color, size=12, no_wrap=True, overflow=ft.TextOverflow.ELLIPSIS)),
                ft.DataCell(ft.Text(self._truncate(part_text), size=12, no_wrap=True, overflow=ft.TextOverflow.ELLIPSIS)),
                ft.DataCell(ft.Text(self._truncate(row.location or '-'), size=12, no_wrap=True, overflow=ft.TextOverflow.ELLIPSIS)),
    ...
    def _get_location_pk(self, location_value: str) -> int:
        return inventree_interface.resolve_stock_location_pk(location_value, self.stock_location_id_map)
    ...
    def _on_apply(self, _):
        ...
        def _process_row(idx: int, row: ExistingPartScanRow) -> Dict[str, Any]:
            ...
            try:
                ...
                if sp_list_resp is not None:
                    ...
                    sp_rows = sp_payload.get('results', sp_payload) if isinstance(sp_payload, dict) else sp_payload
                    for sp in (sp_rows if isinstance(sp_rows, list) else []):
                        ...
                        if sp_resp is None or sp_resp.status_code not in [200, 202]:
                            errors.append(f'{row.part_name}: supplier part pk={sp_pk} location update failed')
                ...
                while stock_url:
                    ...
                    for si in (s_rows if isinstance(s_rows, list) else []):
                        try:
                            if si.get('location') is not None and int(si['location']) == location_pk:
                                continue
                        except Exception:
                            pass
                ...
                if stock_items:
                    ...
                    tr = self._request_with_retries(
                        ...
                        headers=headers,
                        json={'items': transfer_items, 'location': location_pk, 'notes': 'Ki-nTree location update'},
                        timeout=30,
                    )
                    if tr is None or tr.status_code not in [200, 201, 202]:
                        ...
                        for si in stock_items:
                            ...
                            if fr is None or fr.status_code not in [200, 202]:
                                errors.append(f'{row.part_name}: stock item pk={si_pk} transfer failed')
        ...
        if successful_row_ids:
            with self._rows_lock:
                self.scanned_rows = [r for r in self.scanned_rows if r.row_id not in successful_row_ids]
            self._update_results_table()
