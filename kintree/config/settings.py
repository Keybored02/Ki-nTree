...
def load_user_config():
    ...
    # Create user files
    return config_interface.load_user_config_files(
        path_to_root=CONFIG_ROOT,
...
# Compact layout (InvenTree / KiCad / Create embedded in Part Search)
COMPACT_LAYOUT = CONFIG_GENERAL.get("COMPACT_LAYOUT", False)
# Default Supplier
...
def load_suppliers():
    ...
    if update_supplier_config:
        config_interface.dump_file(
            {**CONFIG_SUPPLIERS, **update_supplier_config}, CONFIG_SUPPLIERS_PATH
        )
        CONFIG_SUPPLIERS = config_interface.load_file(CONFIG_SUPPLIERS_PATH)
        return False
    return True
...
CONFIG_ELEMENT14 = config_interface.load_file(
    os.path.join(CONFIG_USER_FILES, "element14_config.yaml")
)
...
CONFIG_AUTOMATIONDIRECT = config_interface.load_file(
    os.path.join(CONFIG_USER_FILES, "automationdirect_config.yaml")
)
...
def load_cache_settings():
    ...
    # Part images
    search_datasheets = os.path.join(USER_SETTINGS["USER_CACHE"], "datasheets", "")
    # Create folder if it does not exists
...
def load_kicad_settings():
    ...
    if kicad_user_settings:
        ...
        KICAD_SETTINGS["KICAD_TEMPLATES_PATH"] = kicad_user_settings.get(
            "KICAD_TEMPLATES_PATH", None
        )
        KICAD_SETTINGS["KICAD_FOOTPRINTS_PATH"] = kicad_user_settings.get(
...
symbol_template_lib = os.path.join(PROJECT_DIR, "kicad", "templates", "library_template.kicad_sym")
