...
def load_user_config_files(path_to_root: str, path_to_user_files: str, silent=True) -> bool:
        ...
        for template_file in os.listdir(path):
            ...
            try:
                ...
                if list(template_data.keys()) == list(user_data.keys()):
                    ...
                    if template_vs_user:
                        print(
                            f'[INFO]\tTEMPLATE "{filename}" configuration file contains the following keys which are NOT in your user settings: {template_vs_user}'
                        )
                    # if user_vs_template:
    ...
    for dir in [
        ...
        try:
            ...
            cprint(
                f"[INFO]\tWarning: Failed to load {dir.title()} configuration",
                silent=silent,
            )
            result = False
...
def save_inventree_user_settings(
    enable: bool,
    server: str,
    username: str,
    password: str,
...
def load_libraries_paths(user_config_path: str, library_path: str) -> dict:
            ...
            if libraries:
                for library in libraries:
                    if library in found_library_files:
                        symbol_libraries_paths[category][library] = (
                            library_path + library + ".kicad_sym"
                        )
                        assigned_files.append(library)
    except Exception:
        import logging

        logging.exception("Exception assigning files to symbol libraries:")
    ...
    try:
        ...
        logging.exception("Exception sorting uncategorized symbol libraries:")

    # Check that library paths are loaded
    path_loaded = False
    for _category, paths in symbol_libraries_paths.items():
        if paths:
            path_loaded = True
            break
    if not path_loaded:
    ...
    try:
        for category in user_settings["KICAD_TEMPLATES"].keys():
            for subcategory, file_name in user_settings["KICAD_TEMPLATES"][category].items():
                ...
                if file_name:
                    try:
                        symbol_templates_paths[category][subcategory] = (
                            template_path + file_name + ".kicad_sym"
                        )
                    except KeyError:
        ...
        logging.exception("Exception assigning symbol templates:")
        ...
        for category, libraries in user_settings["KICAD_FOOTPRINTS"].items():
            ...
            if libraries:
                for folder in libraries:
                    footprint_libraries_paths[category][folder] = (
                        footprint_path + folder + ".pretty"
                    )
                    assigned_folders.append(folder)
    except Exception:
        import logging

        logging.exception("Exception assigning folders to footprint libraries:")
        ...
        for category in supplier_categories:
            for subcategory in supplier_categories[category]:
                if FUNCTION_FILTER_KEY in subcategory:
                    clean_supplier_categories[category][
                        subcategory.replace(FUNCTION_FILTER_KEY, "")
...
def sync_inventree_supplier_categories(
    inventree_config_path: str, supplier_config_path: str
) -> dict:
    ...
    try:
        ...
        logging.exception("Exception syncing supplier categories:")
    ...
    for category in categories.keys():
        for user_subcategory, supplier_category in categories[category].items():
            try:
                ...
                logging.exception("Exception assigning supplier category dict:")
            ...
            if inventree_subcategory_filter in supplier_category_keys:
                try:
                    if (
                        supplier_category
                        not in supplier_categories[category][inventree_subcategory_filter]
                    ):
                    ...
                    logging.exception("Exception appending to function filtered supplier category:")
                ...
                try:
                    ...
                    logging.exception("Exception assigning function filtered supplier category:")
            else:
                try:
                    ...
                    logging.exception("Exception appending to user_subcategory supplier category:")
                ...
                try:
                    ...
                    logging.exception("Exception assigning user_subcategory supplier category:")
