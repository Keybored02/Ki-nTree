...
def tme_api_request(
    ...
    **kwargs,
    ...
    if not TME_API_TOKEN and not TME_API_SECRET:
        ...
        cprint(
            "[INFO]\tWarning: Value not found for TME_API_TOKEN and/or TME_API_SECRET",
            silent=False,
        )
        return None
    ...
    signature_base = (
        "POST" + "&" + urllib.parse.quote(url, "") + "&" + urllib.parse.quote(encoded_params, "")
    )
    hmac_value = hmac.new(TME_API_SECRET.encode(), signature_base.encode(), hashlib.sha1).digest()
...
def fetch_part_info(part_number: str, resolve_linked_documents: bool = False) -> dict:
    ...
    def fetch_datasheet_from_product_files() -> str:
        ...
        files_response = tme_api_query(
            tme_api_request("/Products/GetProductsFiles", tme_api_settings, params)
        )
        ...
        for doc_type in preferred_order:
            for doc in document_list:
                ...
                document_name = str(
                    doc.get("DocumentName", "") or doc.get("Name", "") or doc.get("Title", "")
                ).lower()
                document_url = normalize_file_url(
                    doc.get("DocumentUrl", "") or doc.get("Url", "") or doc.get("Link", "")
                )
                if (
                    document_type == doc_type
                    or doc_type.lower() in document_name
                    or doc_type.lower() in document_url.lower()
                ):
                    return (
                        resolve_linked_document_url(document_url)
    ...
    if response is not None and response.get("Status") == "OK":
        ...
        if found:
            for param in response["Data"]["ProductList"][index].get("ParameterList", []):
                part_info["parameters"][param.get("ParameterName", "")] = param.get(
                    "ParameterValue", ""
                )
