...
def parse_snapeda_response(response: dict) -> dict:
    ...
    if number_results == 1:
        try:
            ...
            data["part_url"] = SNAPEDA_URL + response["results"][0]["_links"]["self"].get(
                "href", ""
            )
        ...
        try:
            data["symbol_image"] = response["results"][0]["models"][0]["symbol_medium"].get(
                "url", None
            )
        except KeyError:
            pass
        try:
            data["footprint_image"] = response["results"][0]["models"][0]["package_medium"].get(
                "url", None
            )
        except KeyError:
            pass
        ...
        try:
            data["part_url"] = (
                SNAPEDA_URL
                + "/search/"
                + response["pages"][0].get("link", None).split("&")[0]
                + "&ref=kintree"
            )
        except Exception:
            import logging

            logging.exception("Exception extracting SnapEDA part_url:")
    else:
        pass
