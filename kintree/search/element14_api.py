...
def get_default_store_url(supplier: str) -> str:
    ...
    if not default_store:
        ...
        cprint(
            f'[INFO]\tWarning: Default store "{supplier.upper()}_STORE" value not configured',
            silent=False,
        )
...
def build_api_url(part_number: str, supplier: str, store_url=None, silent=False) -> str:
    ...
    if not api_key:
        ...
        cprint(
            "[INFO]\tWarning: ELEMENT14_PRODUCT_SEARCH_API_KEY user value not configured",
            silent=silent,
        )
        ...
        if not api_key:
            cprint(
                "[INFO]\tWarning: ELEMENT14_PRODUCT_SEARCH_API_KEY env variable value not found",
                silent=False,
            )
    if not store_url:
    ...
    for key in part:
        if key in headers:
            if key == "displayName":
                # String to remove
                str_remove = (
                    part["brandName"] + " - " + part["translatedManufacturerPartNumber"] + " - "
                )
                # Remove and limit to 100 chars
                part_info["displayName"] = part["displayName"].replace(str_remove, "")[:100]
            elif key == "datasheets":
                try:
                    part_info["datasheet_url"] = part["datasheets"][0]["url"].replace(
                        "http", "https"
                    )
                except IndexError:
                    pass
