...
def fetch_part_info(part_number: str, silent=False) -> dict:
    ...
    def search_timeout(timeout=10):
        url = (
            automationdirect_api_settings.get("AUTOMATIONDIRECT_API_URL", "")
            + automationdirect_api_settings.get("AUTOMATIONDIRECT_API_SEARCH_QUERY", "")
            + part_number
            + automationdirect_api_settings.get("AUTOMATIONDIRECT_API_SEARCH_STRING", "")
            + part_number
        )
    ...
    try:
        ...
        if part["numFound"] > 0:
            if part["numFound"] == 1:
                ...
                cprint(
                    f'[INFO]\tFound {part["numFound"]} results for "{part_number}", selecting first result',
                    silent=False,
                )
    ...
    for key in part:
        if key in headers:
            if key == "image_file_name":  # JSON only returns image name, need to add path
                try:
                    part_info[key] = (
                        automationdirect_api_settings.get("AUTOMATIONDIRECT_API_IMAGE_PATH", "")
                        + part["image_file_name"]
                    )
                except IndexError:
                    pass
                ...
                try:
                    part_info[key] = (
                        automationdirect_api_settings.get("AUTOMATIONDIRECT_API_ROOT_URL", "")
                        + part["spec_url"]
                    )
                except IndexError:
                    pass
                ...
                key == "manual_url"
            ):  # manul url returns .html file name, need to build the rest of the URL
                try:
                    part_info[key] = (
                        automationdirect_api_settings.get("AUTOMATIONDIRECT_API_ROOT_URL", "")
                        + "/static/manuals/"
                        + str(part["manual_url"]).rsplit(".", 1)[0]
                        + "/"
                        + part["manual_url"]
                    )
                except IndexError:
                ...
                key == "url_fullpath"
            ):  # despite being named fullpath, the URL needs the TLD as a prefix
                try:
                    part_info[key] = (
                        automationdirect_api_settings.get("AUTOMATIONDIRECT_API_ROOT_URL", "")
                        + "/adc/shopping"
                        + str(part["url_fullpath"])
                    )
                except IndexError:
                    pass
    ...
    if part.get(parameter_key, ""):
        for attribute in part[parameter_key]:
            ...
            ):  # Manufacturer Name returned as a parameter, pick it out of parameters list aand store it appropriately
    ...
    try:
        ...
        cprint(
            f'[INFO]\tNo pricing attribute "{e.args[0]}" found for "{part_number}"',
            silent=silent,
        )
