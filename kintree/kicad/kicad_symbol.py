...
class ComponentLibManager(object):
    def __init__(self, library_path):
        ...
        if not os.path.isfile(library_path):
            cprint(
                f"[KCAD]\tError loading library file ({library_path})",
        ...
        cprint(
            "[KCAD]\tNumber of parts in library "
            + self.library_name
            + ": "
            + str(len(self.kicad_lib.symbols)),
            silent=settings.SILENT,
        )
    ...
    def is_symbol_in_library(self, symbol_id):
        ...
        for symbol in self.kicad_lib.symbols:
            ...
            if symbol.libId == symbol_id:
                cprint(
                    f"[KCAD]\tWarning: Component {symbol_id} already in library",
                    silent=settings.SILENT,
                )
                return True
    ...
    def add_symbol_to_library_from_inventree(
        self, symbol_data, template_path=None, show_progress=True
    ):
        ...
        if not symbol_id:
            cprint(
                "[KCAD] Error: Adding a new symbol to a KiCad library requires the 'Symbol' key with the following format: {lib}:{symbol_id}"
            )
        ...
        if not os.path.isfile(template_path):
            cprint(
                f"[KCAD]\tError loading template file ({template_path})",
                silent=settings.SILENT,
            )
            return part_in_lib, new_part, part_name
        ...
        if len(templatelib.symbols) == 1:
            ...
            cprint(
                "[KCAD]\tError: Found more than 1 symbol template in template file, aborting",
                silent=settings.SILENT,
            )
            return part_in_lib, new_part, part_name
        ...
        cprint(
            f"[KCAD]\tSuccess: Component added to library {self.library_name}",
            silent=settings.SILENT,
        )
        part_in_lib = True
        new_part = True
