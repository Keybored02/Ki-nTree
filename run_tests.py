...
from kintree.common.tools import create_library
from kintree.common.tools import download_with_retry
...
from kintree.database import inventree_interface
from kintree.kicad import kicad_interface
from kintree.search import automationdirect_api
from kintree.search import digikey_api
from kintree.search import element14_api
from kintree.search import jameco_api
from kintree.search import lcsc_api
from kintree.search import mouser_api
from kintree.search import snapeda_api
...
footprint_libraries_test_path = os.path.join(
    settings.PROJECT_DIR, "tests", "files", "FOOTPRINTS", ""
)
...
if ENABLE_API:
    ...
    if "Element14" in settings.SUPPORTED_SUPPLIERS_API:
        for _i in range(2):
            ...
            if not element14_api.test_api() or not element14_api.test_api(
...
# Load test samples
samples = config_interface.load_file(os.path.abspath(os.path.join("tests", "test_samples.yaml")))
PART_TEST_SAMPLES = {}
    ...
    if settings.ENABLE_TEST:
        ...
        if ENABLE_KICAD or ENABLE_INVENTREE:
            for category in PART_TEST_SAMPLES.keys():
                ...
                if (
                    ENABLE_KICAD
                    and ENABLE_INVENTREE
                    and category == list(PART_TEST_SAMPLES.keys())[-1]
                ):
                    last_category = True
                ...
                for number, status in PART_TEST_SAMPLES[category].items():
                    ...
                    # Update categories
                    part_info["category_tree"] = (
                        inventree_interface.get_categories_from_supplier_data(part_info)
                    )
                    # Needed for tests
                    ...
                    if ENABLE_KICAD:
                        ...
                        kicad_result, kicad_new_part, kicad_part_name = (
        ...
        if ENABLE_TEST_METHODS and ENABLE_INVENTREE:
            ...
            for method_idx, method_name in enumerate(methods):
                ...
                if method_idx == 0:
                    ...
                    if not inventree_interface.translate_form_to_inventree(
                        part_info, categories, is_custom=True
                    ):
                        method_success = False

                elif method_idx == 2:
                    # Digi-Key search missing part number
                    search = inventree_interface.supplier_search(
                    ...
                    symbol_libraries_paths = config_interface.load_libraries_paths(
                        settings.KICAD_CONFIG_CATEGORY_MAP, symbol_libraries_test_path
                    )
                    footprint_libraries_paths = config_interface.load_footprint_paths(
                        settings.KICAD_CONFIG_CATEGORY_MAP,
                    ...
                    # Add footprint library to user file
                    add_footprint_lib = config_interface.add_footprint_library(
                        user_config_path=settings.KICAD_CONFIG_CATEGORY_MAP,
                        category="category_test",
                    ...
                    test_image_requestslib = (
                        "https://www.newark.com/productimages/standard/en_GB/GE2SOD12307-40.jpg"
                    )
                    ...
                    if not download_with_retry(
                        test_image_urllib, "./image1.jpg", silent=True, filetype="Image"
                    ):
                        print(" [1] ")
                        method_success = False
                    if not download_with_retry(
                    ...
                    # Test PDF
                    if not download_with_retry(
                        test_pdf_urllib, "./datasheet.pdf", silent=True, filetype="PDF"
                    ):
                        print(" [3] ")
                        method_success = False
                    # Wrong folder
                    ...
                    if inventree_interface.inventree_create_alternate(
                        part_info=part_info,
                    ...
                    if method_success:
                        ...
                        cat_branch = inventree_interface.build_category_tree(
