#!/usr/bin/env python3
"""
Bulk create nested stock locations (racks → shelves → boxes) in InvenTree with location types.

Uses InvenTree stock location API endpoints:
  GET  /api/stock/location/  (list locations)
  GET  /api/stock/location-type/  (list location types)
  POST /api/stock/location/  (create location)

Examples:
  python bulk_create_locations_v2.py --config location_config.yaml --dry-run
  python bulk_create_locations_v2.py --config location_config.yaml --confirm
  python bulk_create_locations_v2.py --config location_config.yaml --parent-path "Genesy/Taverna" --confirm

Location config YAML format:
  racks: 9
  shelves_per_rack: 6
  boxes_per_shelf: 20
  location_types:
    rack: "Rack"      # or pk: 2
    shelf: "Shelf"    # or pk: 3
    box: "Box"        # or pk: 4
  naming_pattern:
    rack: "R{i:02d}"
    shelf: "R{rack:02d}-L{i:02d}"
    box: "R{rack:02d}-L{shelf:02d}-B{i:02d}"
"""

from __future__ import annotations

import argparse
from typing import Any, Dict, List, Optional

import requests
import yaml

from kintree.config import settings
from kintree.database import inventree_api


def _build_auth_headers(api_obj) -> Dict[str, str]:
    """Build authorization headers for InvenTree API requests."""
    token = getattr(api_obj, "token", None)
    if not token:
        raise RuntimeError("InvenTree auth token is missing after login")
    return {
        "Authorization": f"Token {token}",
        "Content-Type": "application/json",
        "Accept": "application/json",
    }


def _api_url(base_url: str, path: str) -> str:
    """Construct full API URL."""
    return f"{base_url.rstrip('/')}/{path.lstrip('/')}"


def fetch_location_types(
    base_url: str,
    headers: Dict[str, str],
    timeout: int = 30,
) -> List[Dict[str, Any]]:
    """Fetch available location types from InvenTree API."""
    url = _api_url(base_url, "api/stock/location-type/")
    response = requests.get(url, headers=headers, timeout=timeout)
    response.raise_for_status()
    # This endpoint returns plain list, not paginated
    return response.json() if isinstance(response.json(), list) else []


def resolve_location_type_pk(
    location_type_spec: Optional[str],
    location_types: List[Dict[str, Any]],
) -> Optional[int]:
    """
    Resolve location type name or pk to pk.

    Returns pk if found, None if not specified.
    """
    if not location_type_spec:
        return None

    # Try as pk first
    try:
        return int(str(location_type_spec).strip())
    except ValueError:
        pass

    # Search by name
    spec_lower = str(location_type_spec).strip().lower()
    for loc_type in location_types:
        if loc_type.get("name", "").lower() == spec_lower:
            return loc_type.get("pk")

    raise ValueError(f"Location type not found: {location_type_spec}")


def print_location_types(location_types: List[Dict[str, Any]]) -> None:
    """Pretty print available location types."""
    if not location_types:
        print("No location types found")
        return

    print("Available Location Types:")
    print("-" * 60)
    for loc_type in location_types:
        pk = loc_type.get("pk")
        name = loc_type.get("name", "")
        description = loc_type.get("description", "")
        print(f"  pk={pk} | name='{name}' | description='{description}'")
    print("-" * 60)


def find_location_by_path(
    base_url: str,
    headers: Dict[str, str],
    path: str,
    timeout: int = 30,
) -> Optional[Dict[str, Any]]:
    """
    Find a stock location by its full path (e.g. "Genesy / Taverna").

    Returns the location dict with pk and metadata, or None if not found.
    """
    all_locations: List[Dict[str, Any]] = []
    url = _api_url(base_url, "api/stock/location/")

    # Fetch all locations
    while url:
        response = requests.get(url, headers=headers, timeout=timeout)
        response.raise_for_status()
        data = response.json()

        if isinstance(data, dict) and "results" in data:
            results = data.get("results") or []
            all_locations.extend(results)
            url = data.get("next")
        elif isinstance(data, list):
            all_locations.extend(data)
            break

    # Build location map: pk -> location object
    location_map: Dict[int, Dict[str, Any]] = {}
    for loc in all_locations:
        pk = loc.get("pk")
        if pk:
            location_map[int(pk)] = loc

    # Build path map: full_path -> pk
    path_map: Dict[str, int] = {}
    memo: Dict[int, str] = {}

    def build_path(loc_pk: int) -> str:
        """Recursively build full path for a location."""
        if loc_pk in memo:
            return memo[loc_pk]

        loc = location_map.get(loc_pk)
        if not loc:
            return ""

        name = loc.get("name", "")
        parent = loc.get("parent")

        if parent is None:
            result = name
        else:
            parent_path = build_path(int(parent))
            result = f"{parent_path} / {name}" if parent_path else name

        memo[loc_pk] = result
        return result

    # Build complete path map
    for loc_pk in location_map.keys():
        full_path = build_path(loc_pk)
        path_map[full_path] = loc_pk

    # Search for exact match
    search_path = path.strip()
    if search_path in path_map:
        return location_map[path_map[search_path]]

    return None


def create_location(
    base_url: str,
    headers: Dict[str, str],
    name: str,
    description: str = "",
    parent_pk: Optional[int] = None,
    location_type_pk: Optional[int] = None,
    timeout: int = 30,
) -> Optional[Dict[str, Any]]:
    """
    Create a new stock location via API.

    Returns the created location object with pk, or None on error.
    """
    payload: Dict[str, Any] = {
        "name": name,
    }
    if description:
        payload["description"] = description
    if parent_pk is not None:
        payload["parent"] = parent_pk
    if location_type_pk is not None:
        payload["location_type"] = location_type_pk

    endpoint = _api_url(base_url, "api/stock/location/")
    response = requests.post(endpoint, headers=headers, json=payload, timeout=timeout)

    if response.status_code in [200, 201]:
        return response.json()
    else:
        raise RuntimeError(
            f"Failed to create location '{name}': "
            f"HTTP {response.status_code}: {response.text[:500]}"
        )


def barcode_string_for_location(location_row: Dict[str, Any]) -> str:
    """Build barcode input string according to hierarchy rules."""
    level = str(location_row.get("level") or "").lower()
    name = str(location_row.get("name") or "")
    rack_id = int(location_row.get("rack_id") or 0)
    shelf_id = int(location_row.get("shelf_id") or 0)

    if level == "rack":
        # Rack barcode uses rack name directly, e.g. R01.
        return name

    if level == "shelf":
        # Shelf barcode uses progressive rack+shelf, e.g. R01-L01.
        return f"R{rack_id:02d}-L{shelf_id:02d}"

    if level == "box":
        # Box barcode uses the box name, e.g. R01-L01-B01.
        return name

    return name


def link_barcode_to_location(
    base_url: str,
    headers: Dict[str, str],
    barcode: str,
    stocklocation_pk: int,
    timeout: int = 30,
) -> Dict[str, Any]:
    """Assign barcode to stock location through barcode link endpoint."""
    endpoint = _api_url(base_url, "api/barcode/link/")
    payload = {
        "barcode": barcode,
        "stocklocation": int(stocklocation_pk),
    }
    response = requests.post(endpoint, headers=headers, json=payload, timeout=timeout)

    if response.status_code in [200, 201]:
        data = response.json()
        if isinstance(data, dict):
            return data
        return {"barcode": barcode, "stocklocation": stocklocation_pk}

    raise RuntimeError(
        f"Failed barcode link for location pk={stocklocation_pk} barcode='{barcode}': "
        f"HTTP {response.status_code}: {response.text[:500]}"
    )


def load_config(config_path: str) -> Dict[str, Any]:
    """Load location configuration from YAML file."""
    with open(config_path, "r") as f:
        config = yaml.safe_load(f)
    return config or {}


def validate_config(config: Dict[str, Any]) -> None:
    """Validate required fields in config."""
    required = ["racks", "shelves_per_rack", "boxes_per_shelf"]
    for field in required:
        if field not in config:
            raise ValueError(f"Missing required config field: {field}")


def build_location_tree(config: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Build a list of locations to create based on config.

    Returns list of dicts with keys: name, description, level (rack/shelf/box), parent_idx, location_type_key
    """
    locations: List[Dict[str, Any]] = []

    racks = config.get("racks", 0)
    shelves_per_rack = config.get("shelves_per_rack", 0)
    boxes_per_shelf = config.get("boxes_per_shelf", 0)
    rack_offset = config.get("rack_offset", 0)

    naming_pattern = config.get("naming_pattern", {})
    rack_pattern = naming_pattern.get("rack", "RACK-{i:02d}")
    shelf_pattern = naming_pattern.get("shelf", "RACK-{rack}/SHELF-{i:02d}")
    box_pattern = naming_pattern.get("box", "RACK-{rack}/SHELF-{shelf}/BOX-{i:02d}")

    # Build rack locations
    rack_indices: Dict[int, int] = {}  # rack_id -> index in locations
    for rack_idx in range(1, racks + 1):
        display_idx = rack_idx + rack_offset
        rack_name = rack_pattern.format(i=display_idx, rack=display_idx)
        rack_indices[rack_idx] = len(locations)
        locations.append(
            {
                "name": rack_name,
                "description": f"Box {display_idx:02d}",
                "level": "rack",
                "parent_idx": None,
                "location_type_key": "rack",
                "rack_id": display_idx,
                "shelf_id": None,
                "box_id": None,
            }
        )

    # Build shelf locations
    shelf_indices: Dict[tuple, int] = {}  # (rack_idx, shelf_id) -> index in locations
    for rack_idx in range(1, racks + 1):
        display_rack_idx = rack_idx + rack_offset
        for shelf_idx in range(1, shelves_per_rack + 1):
            shelf_name = shelf_pattern.format(
                rack=display_rack_idx, i=shelf_idx, shelf=shelf_idx
            )
            parent_rack_idx = rack_indices[rack_idx]
            shelf_indices[(rack_idx, shelf_idx)] = len(locations)
            locations.append(
                {
                    "name": shelf_name,
                    "description": f"Box {shelf_idx:02d}",
                    "level": "shelf",
                    "parent_idx": parent_rack_idx,
                    "location_type_key": "shelf",
                    "rack_id": display_rack_idx,
                    "shelf_id": shelf_idx,
                    "box_id": None,
                }
            )

    # Build box locations
    for rack_idx in range(1, racks + 1):
        display_rack_idx = rack_idx + rack_offset
        for shelf_idx in range(1, shelves_per_rack + 1):
            for box_idx in range(1, boxes_per_shelf + 1):
                box_name = box_pattern.format(
                    rack=display_rack_idx, shelf=shelf_idx, i=box_idx
                )
                parent_shelf_idx = shelf_indices[(rack_idx, shelf_idx)]
                locations.append(
                    {
                        "name": box_name,
                        "description": f"Box {box_idx:02d}",
                        "level": "box",
                        "parent_idx": parent_shelf_idx,
                        "location_type_key": "box",
                        "rack_id": display_rack_idx,
                        "shelf_id": shelf_idx,
                        "box_id": box_idx,
                    }
                )

    return locations


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Bulk create nested stock locations in InvenTree with location types",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    parser.add_argument(
        "--config", required=True, help="YAML config file defining location hierarchy"
    )
    parser.add_argument(
        "--parent-path",
        help="Parent location path (e.g. 'Genesy / Storage'). If not specified, creates at root.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Preview locations to be created; do not create",
    )
    parser.add_argument(
        "--confirm", action="store_true", help="Actually execute location creation"
    )
    parser.add_argument(
        "--timeout", type=int, default=30, help="HTTP timeout in seconds"
    )

    args = parser.parse_args()

    if not args.dry_run and not args.confirm:
        print("ERROR: refusing to run without --dry-run or --confirm")
        print("Tip: start with --dry-run to preview")
        return 2

    # Load InvenTree credentials
    settings.load_inventree_settings()

    if not settings.SERVER_ADDRESS:
        print("ERROR: Missing InvenTree SERVER_ADDRESS in config")
        return 2

    if not settings.USERNAME or not settings.PASSWORD:
        print("ERROR: Missing InvenTree USERNAME/PASSWORD in config")
        return 2

    print(f"Connecting to InvenTree: {settings.SERVER_ADDRESS}")
    connected = inventree_api.connect(
        server=settings.SERVER_ADDRESS,
        username=settings.USERNAME,
        password=settings.PASSWORD,
        connect_timeout=max(5, args.timeout),
        silent=False,
        proxies=settings.PROXIES if getattr(settings, "ENABLE_PROXY", False) else None,
    )

    if not connected:
        print("ERROR: Could not authenticate to InvenTree")
        return 1

    api_obj = inventree_api.inventree_api
    headers = _build_auth_headers(api_obj)
    base_url = getattr(api_obj, "base_url", settings.SERVER_ADDRESS)

    # Fetch location types
    try:
        location_types = fetch_location_types(base_url, headers, timeout=args.timeout)
        print(f"Available location types: {len(location_types)}")
    except Exception as exc:
        print(f"ERROR: failed to fetch location types: {repr(exc)}")
        return 1

    # Load config
    try:
        config = load_config(args.config)
        validate_config(config)
    except FileNotFoundError:
        print(f"ERROR: config file not found: {args.config}")
        return 2
    except ValueError as exc:
        print(f"ERROR: invalid config: {exc}")
        return 2
    except yaml.YAMLError as exc:
        print(f"ERROR: failed to parse YAML: {exc}")
        return 2
    except Exception as exc:
        print(f"ERROR: unexpected error loading config: {repr(exc)}")
        return 1

    # Resolve location type PKs
    location_type_pks: Dict[str, Optional[int]] = {}
    location_types_config = config.get("location_types", {})
    try:
        for key in ["rack", "shelf", "box"]:
            spec = location_types_config.get(key)
            location_type_pks[key] = resolve_location_type_pk(spec, location_types)
            if location_type_pks[key]:
                print(f"  {key}: {spec} (pk={location_type_pks[key]})")
    except ValueError as exc:
        print(f"ERROR: {exc}")
        print_location_types(location_types)
        return 2

    # Build location tree
    try:
        locations_to_create = build_location_tree(config)
    except Exception as exc:
        print(f"ERROR: failed to build location tree: {repr(exc)}")
        return 1

    # Resolve parent location
    parent_pk: Optional[int] = None

    if args.parent_path:
        try:
            parent_loc = find_location_by_path(
                base_url, headers, args.parent_path, timeout=args.timeout
            )
            if not parent_loc:
                print(f"ERROR: parent location not found: {args.parent_path}")
                return 2
            parent_pk = parent_loc.get("pk")
            print(f"Resolved parent location: {args.parent_path} (pk={parent_pk})")
        except Exception as exc:
            print(f"ERROR: failed to resolve parent location: {repr(exc)}")
            return 1

    # Print preview
    print(f"\nPreview: {len(locations_to_create)} locations to create")
    print("-" * 100)

    for idx, loc in enumerate(locations_to_create[:25]):
        parent_idx = loc.get("parent_idx")
        parent_name = (
            locations_to_create[parent_idx]["name"]
            if parent_idx is not None
            else "(root)"
        )
        level = loc.get("level")
        name = loc["name"]
        desc = loc.get("description", "")
        barcode = barcode_string_for_location(loc)
        print(
            f"  [{level:5s}] {name:20s} parent={parent_name:20s} barcode={barcode:15s} desc='{desc}'"
        )

    if len(locations_to_create) > 25:
        print(f"  ... and {len(locations_to_create) - 25} more")

    print("-" * 100)

    if args.dry_run:
        print("Dry-run complete. No locations created.")
        return 0

    # Execute creation
    print("\nCreating locations...")
    created_pks: Dict[int, int] = {}  # Map location list index -> API pk

    for idx, loc in enumerate(locations_to_create):
        name = loc["name"]
        description = loc.get("description", "")
        barcode = barcode_string_for_location(loc)
        location_type_key = loc.get("location_type_key")
        loc_type_pk = location_type_pks.get(location_type_key)

        # Resolve parent pk
        parent_idx = loc.get("parent_idx")
        loc_parent_pk: Optional[int] = None
        if parent_idx is not None:
            loc_parent_pk = created_pks.get(parent_idx)
        elif parent_pk is not None:
            loc_parent_pk = parent_pk

        try:
            created_loc = create_location(
                base_url,
                headers,
                name,
                description=description,
                parent_pk=loc_parent_pk,
                location_type_pk=loc_type_pk,
                timeout=args.timeout,
            )
            created_pk = created_loc.get("pk")
            if created_pk is None:
                raise RuntimeError(f"Create response missing pk for location '{name}'")

            # Link barcode immediately at creation time.
            link_barcode_to_location(
                base_url,
                headers,
                barcode=barcode,
                stocklocation_pk=int(created_pk),
                timeout=args.timeout,
            )

            created_pks[idx] = created_pk

            # Print progress
            if idx % 50 == 0 or idx == len(locations_to_create) - 1:
                print(f"  Progress: {idx + 1}/{len(locations_to_create)} created...")
        except Exception as exc:
            print(f"  ✗ Failed: {name}")
            print(f"    {repr(exc)}")
            return 1

    print(f"\nSuccessfully created {len(created_pks)} locations!")
    return 0


if __name__ == "__main__":
    try:
        raise SystemExit(main())
    except KeyboardInterrupt:
        print("Cancelled by user")
        raise SystemExit(130)
